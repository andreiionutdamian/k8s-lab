...
func GetAllListsHandler(rw http.ResponseWriter, req *http.Request) {
	rw.Header().Set("Content-Type", "application/json")
	data, err := json.MarshalIndent(
		lists, // this is the in-memory map of lists
		"",    // prefix good for logging
		"  ",  // indent
	)
	if err != nil {
		data = []byte("Error generating full db: " + err.Error())
	}
	rw.Write(data)
}
...
func main() {
	...
	// Setup our HTTP routes. The root / is handled by default
	r := mux.NewRouter()
	...
	r.Path("/all").Methods("GET").HandlerFunc(GetAllListsHandler)
